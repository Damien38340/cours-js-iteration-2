 ...
 */
function types() {
    const allTypes = Object.entries(data['types']);
 ...
 */
function formats() {
    const formatsArray = Object.entries(data['data_formats']);
...
function get_objects_by_operator(operator) {
    ...
    const objectsByOperator = {objects: data.objects.filter(obj => obj.provisionning.operator.includes(operator))};
    if (objectsByOperator.objects.length > 0) {
        return objectsByOperator;
    }
    return undefined;
}
...
function get_types_by_comm(comm) {
    const typesByComm = {types: Object.values(data.types).filter(obj => obj.communication.includes(comm))};
    if (typesByComm.types.length > 0) {
        return typesByComm;
    }
    return undefined;
}
...
function get_types_by_format(format) {
    const typesByFormat = {
        types: Object.entries(data.data_formats).filter(([key, value]) => value.data_type === format)
    };
    console.log(typesByFormat);
    if (typesByFormat.types.length > 0) {
